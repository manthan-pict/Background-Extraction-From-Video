...
def stabilizeVideo(input_video_path):
    # Read input video
    filename = os.path.basename(input_video_path)
    files = filename.split(".")
    cp = cv2.VideoCapture(input_video_path)
    ...
    for i in range(n_frames - 2):
        ...
        if (frame_out.shape[1] > 1920):
            ...
            dim = (w, h)
            frame_out = cv2.resize(frame_out, dim)
...
stabilizeVideo(os.path.join("input_vidoes", "video_stab_input.mp4"))